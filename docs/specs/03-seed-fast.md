...
Populate the `slowquery-fast` Neon branch with the **same rows** as the `slowquery` branch from Spec 02, then create the indexes the slow branch deliberately omits. The two branches must be row-identical so that the `POST /branches/switch` endpoint (Spec 06) produces a visible p95 drop *only* because of indexing — never because the datasets differ. Without that guarantee, any before/after latency story is an artifact of two different datasets, and the whole project loses its credibility.
...
- **Out:** the same row counts as `seed_slow` plus four indexes:
  - `CREATE INDEX ix_orders_user_id ON orders(user_id);` — backs `GET /users/{id}/orders` (`WHERE user_id = ?`).
  - `CREATE INDEX ix_orders_created_at ON orders(created_at);` — backs `GET /orders` (`ORDER BY created_at DESC LIMIT n`).
...
3. The `CREATE INDEX` calls are the **last** statements the script runs. Indexes are created after all rows are inserted because creating indexes up-front and then bulk-inserting is dramatically slower on Postgres.
...
3. The four indexed columns are stored in a module-level `FAST_INDEXES: Final[tuple[str, ...]]` constant and the script emits them in that order.
...
5. On the `slowquery-fast` container, `SELECT indexname FROM pg_indexes WHERE tablename = 'orders'` includes `ix_orders_user_id` and `ix_orders_created_at`; same for the two `order_items` indexes.
6. On the `slowquery` container from Spec 02, none of those four indexes exist.
...
- [ ] `FAST_INDEXES: Final[tuple[str, ...]]` enumerates the four CREATE INDEX statements.
